  ...
  return (
    <div className="flex flex-col items-center justify-between min-h-[100dvh] w-full text-center">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 md:px-10 w-full max-w-md md:max-w-lg lg:max-w-xl mx-auto">
        <div className="relative mb-5 md:mb-7 animate-fade-in-up">
          <Coin size={34} className="absolute -top-5 -left-6 animate-float" />
          <Coin
            size={25}
            className="absolute -top-4 -right-7 animate-float [animation-delay:0.8s]"
          />
          <CoinSmall className="absolute -bottom-3 -left-5 animate-float [animation-delay:1.6s]" />
          <Coin
            size={28}
            className="absolute -bottom-5 -right-4 animate-float [animation-delay:0.4s]"
          />
          ...
          <TitleBadge className="w-60 sm:w-68 md:w-80 lg:w-96" />
        </div>

        {sessionName && (
          <div className="mb-1.5 animate-fade-in-up [animation-delay:0.1s]">
            <span className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary-dark text-xs font-semibold tracking-wide">
              {sessionName}
            </span>
          </div>
        )}
        ...
        {quizName && (
          <div className="mb-2 animate-fade-in-up [animation-delay:0.12s]">
            <span className="inline-block px-3 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[11px] font-medium">
              {quizName}
            </span>
          </div>
        )}

        <p className="text-slate-600 text-sm sm:text-base md:text-lg max-w-xs md:max-w-sm leading-snug mb-6 md:mb-8 animate-fade-in-up [animation-delay:0.15s]">
          Teste seus conhecimentos sobre{" "}
          <strong className="text-primary-dark">educação financeira</strong> e
          ganhe brindes!
        </p>
        ...
        {!sessionName && (
          <p className="mt-4 text-sm text-slate-400 animate-fade-in-up [animation-delay:0.25s]">
            Nenhuma sessão configurada. Acesse as{" "}
            <strong className="text-slate-500">Configurações</strong> para criar
            uma.
          </p>
        )}
      </div>
      ...
      <div className="w-full flex flex-col items-center">
        <button
          onClick={onAdmin}
          className="flex items-center gap-1.5 text-xs md:text-sm text-slate-400 hover:text-slate-600 py-3"
        >
          <Settings size={14} />
          Configurações
        </button>

        <footer className="w-full bg-gradient-to-t from-primary-darker to-primary-dark px-4 py-2 flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
          <div className="flex items-center gap-2 shrink-0">
            <img
              src="/logos/uno_chapeco.svg"
              alt="Unochapecó"
              className="h-7 md:h-9 w-auto"
            />
            <img
              src="/logos/ciencia_computacao_uno.svg"
              alt="Ciência da Computação - Unochapecó"
              className="h-7 md:h-9 w-auto"
            />
            <img
              src="/logos/sistemas_informacao_uno.svg"
              alt="Sistemas de Informação - Unochapecó"
              className="h-7 md:h-9 w-auto"
            />
          </div>
        </footer>
